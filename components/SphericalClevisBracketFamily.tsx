...
export default function SphericalClevisBracketFamily({ subcategory, priceByPartNo = {} }: { subcategory: string; priceByPartNo?: Record<string, number> }) {
  const [q, setQ] = useState("");

  const rows = useMemo(() => {
    const s = q.trim().toLowerCase();
    const baseRows = sphericalClevisBrackets.filter((r) => Object.prototype.hasOwnProperty.call(priceByPartNo, r.part));
    ...
    <section className="w-full">
      ...
      <div className="rounded-2xl border border-slate-200/70 bg-white/70 p-6 shadow-sm backdrop-blur">
        <div className="grid gap-8 md:grid-cols-[1.2fr_1fr] md:items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Spherical Clevis Bracket</h1>
            <p className="mt-3 text-gray-700">
              Spherical clevis brackets. Search a part number to view all dimensions.
            </p>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            ...
            <div className="mt-3 flex items-center justify-between">
              ...
              <a
                href="/specs/spherical-clevis-bracket-diagram.png"
                target="_blank"
      ...
      <div className="mt-8 overflow-hidden rounded-2xl border border-slate-200/70 bg-white/70 shadow-sm backdrop-blur">
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-white/90 backdrop-blur">
              <tr className="border-b border-slate-200">
                ...
                <TipTh label="CD" tip="CD (pin hole diameter)" />
                <TipTh label="CF" tip="CF (clevis feature, see diagram)" />
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.part} className="border-b border-slate-100 hover:bg-slate-50/60">
                  ...
                  <td className="px-4 py-3 text-gray-800">{r.CF}</td>
        ...
        <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
          ...
          <div className="mt-3 grid gap-2 text-sm text-gray-700 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div><strong>CF</strong> — Clevis feature (see diagram)</div>
            <div><strong>CW</strong> — Clevis width</div>
            <div><strong>DD</strong> — Mounting hole diameter</div>
