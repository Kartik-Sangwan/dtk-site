...
export default function Mp4DetachableMountFamily({ subcategory, priceByPartNo = {} }: { subcategory: string; priceByPartNo?: Record<string, number> }) {
  const [q, setQ] = useState("");

  const rows = useMemo(() => {
    const s = q.trim().toLowerCase();
    const baseRows = mp4DetachableMounts.filter((r) => Object.prototype.hasOwnProperty.call(priceByPartNo, r.part));
    ...
    <section className="w-full">
      ...
      <div className="rounded-2xl border border-slate-200/70 bg-white/70 p-6 shadow-sm backdrop-blur">
        <div className="grid gap-8 md:grid-cols-[1.2fr_1fr] md:items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">MP4 Detachable Mount</h1>
            <p className="mt-3 text-gray-700">
              MP4 detachable mounts (cast iron). Search a part number to view dimensions.
              <span className="block mt-2 text-sm text-gray-600">
                Note: add <span className="font-semibold text-gray-900">T</span> for tapped or{" "}
                <span className="font-semibold text-gray-900">H</span> for thru hole at the end of the part number.
              </span>
            </p>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            ...
            <div className="mt-3 flex items-center justify-between">
              ...
              <a
                href="/specs/mp4-detachable-mount-diagram.png"
                target="_blank"
      ...
      <div className="mt-8 overflow-hidden rounded-2xl border border-slate-200/70 bg-white/70 shadow-sm backdrop-blur">
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-white/90 backdrop-blur">
              <tr className="border-b border-slate-200">
                ...
                <TipTh label="KK" tip='Thread size (“DD or KK” in diagram)' />
        ...
        <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
          ...
          <div className="mt-3 grid gap-2 text-sm text-gray-700 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div><strong>D</strong> — D dimension (see diagram)</div>
            <div><strong>ER</strong> — End radius</div>
            <div><strong>G</strong> — G dimension (see diagram)</div>
            <div><strong>KK</strong> — Thread size (DD or KK)</div>
            <div><strong>D-D DIA.</strong> — D-D diameter</div>
          </div>
        </div>
      </div>
    </section>
