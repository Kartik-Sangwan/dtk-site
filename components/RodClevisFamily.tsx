...
export default function RodClevisFamily({ subcategory, priceByPartNo = {} }: { subcategory: string; priceByPartNo?: Record<string, number> }) {
  const [q, setQ] = useState("");

  const rows = useMemo(() => {
    const s = q.trim().toLowerCase();
    const baseRows = rodClevis.filter((r) => Object.prototype.hasOwnProperty.call(priceByPartNo, r.part));
    ...
    <section className="w-full">
      ...
      <div className="rounded-2xl border border-slate-200/70 bg-white/70 p-6 shadow-sm backdrop-blur">
        <div className="grid gap-8 md:grid-cols-[1.2fr_1fr] md:items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Rod Clevis</h1>
            <p className="mt-3 text-gray-700">
              NFPA rod clevis mounts. Search a part number to view all dimensions.
              <span className="block mt-2 text-sm text-gray-600">
                Material: Cast iron (except <span className="font-semibold text-gray-900">07M</span> &{" "}
                <span className="font-semibold text-gray-900">10M</span> which are steel forging).
              </span>
            </p>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            ...
            <div className="mt-3 flex items-center justify-between">
              <div className="text-sm font-semibold text-gray-900">
                Material: Cast iron / steel forging
              </div>
              <a
                href="/specs/rod-clevis-diagram.png"
                target="_blank"
      ...
      <div className="mt-8 overflow-hidden rounded-2xl border border-slate-200/70 bg-white/70 shadow-sm backdrop-blur">
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-white/90 backdrop-blur">
              <tr className="border-b border-slate-200">
                ...
                <TipTh label="CB" tip="Clevis body width (see diagram)" />
                <TipTh label="CD" tip="Pin hole diameter (see diagram)" />
                <TipTh label="CE" tip="Clevis end dimension (see diagram)" />
                <TipTh label="CH" tip="Hex / head dimension (see diagram)" />
                <TipTh label="CW" tip="Clevis width (see diagram)" />
                <TipTh label="F" tip="Flange / thickness dimension (see diagram)" />
                <TipTh label="L" tip="Length dimension (see diagram)" />
                <TipTh label="A" tip="Base dimension (see diagram)" />
                <TipTh label="KK" tip="Thread (tap thru) specification" />
                <TipTh label="ER" tip="End radius (see diagram)" />
            ...
            <tbody>
              {rows.map((r) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-gray-800">{r.CH}</td>
                  <td className="px-4 py-3 text-gray-800">{r.CW}</td>
                  <td className="px-4 py-3 text-gray-800">{r.F}</td>
                  <td className="px-4 py-3 text-gray-800">{r.L}</td>
                  <td className="px-4 py-3 text-gray-800">{r.A}</td>
                  <td className="px-4 py-3 text-gray-800 whitespace-nowrap">{r.KK}</td>
        ...
        <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
          ...
          <div className="mt-3 grid gap-2 text-sm text-gray-700 sm:grid-cols-2 lg:grid-cols-3">
            <div><strong>CB</strong> — Clevis body width</div>
            <div><strong>CD</strong> — Pin hole diameter</div>
            <div><strong>CE</strong> — Clevis end dimension</div>
            <div><strong>CH</strong> — Hex / head dimension</div>
            <div><strong>CW</strong> — Clevis width</div>
            <div><strong>F</strong> — Thickness / flange dimension</div>
            <div><strong>L</strong> — Length dimension</div>
            <div><strong>A</strong> — Base dimension</div>
            <div><strong>KK</strong> — Thread (tap thru)</div>
