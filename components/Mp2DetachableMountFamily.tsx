...
export default function Mp2DetachableMountFamily({ subcategory, priceByPartNo = {} }: { subcategory: string; priceByPartNo?: Record<string, number> }) {
  const [q, setQ] = useState("");

  const rows = useMemo(() => {
    const s = q.trim().toLowerCase();
    const baseRows = mp2Parts.filter((r) => Object.prototype.hasOwnProperty.call(priceByPartNo, r.part));
    ...
    <section className="w-full">
      ...
      <div className="rounded-2xl border border-slate-200/70 bg-white/70 p-6 shadow-sm backdrop-blur">
        <div className="grid gap-8 md:grid-cols-[1.2fr_1fr] md:items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">MP2 Detachable Mount</h1>
            <p className="mt-3 text-gray-700">
              NFPA MP2 detachable mounts. Search a part number to view dimensions.
              <span className="ml-2 text-gray-600">
                (H = thru-hole, T = tapped)
              </span>
            </p>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            ...
            <div className="mt-3 flex items-center justify-between">
              ...
              <a
                href="/specs/mp2-detachable-mount-diagram.png"
                target="_blank"
      ...
      <div className="mt-8 overflow-hidden rounded-2xl border border-slate-200/70 bg-white/70 shadow-sm backdrop-blur">
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-white/90 backdrop-blur">
              <tr className="border-b border-slate-200">
                ...
                <TipTh label="D" tip="D dimension (see diagram)" />
                <TipTh label="ER" tip="ER (end radius)" />
                <TipTh label="G" tip="G dimension (see diagram)" />
                <TipTh label="TAPPED" tip='Thread size if "T" variant (tapped)' />
                <TipTh label="D-D DIA." tip='D-D hole diameter if "H" variant (thru-hole)' />
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.part} className="border-b border-slate-100 hover:bg-slate-50/60">
                  <td className="px-4 py-3 font-semibold text-gray-900 whitespace-nowrap">{r.part}</td>
                  ...
                  <td className="px-4 py-3 text-gray-800">{r.tapped}</td>
                  <td className="px-4 py-3 text-gray-800">{r.ddDia}</td>
        ...
        <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
          ...
          <div className="mt-3 grid gap-2 text-sm text-gray-700 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div><strong>F</strong> — See diagram</div>
            <div><strong>B</strong> — See diagram</div>
            <div><strong>C</strong> — See diagram</div>
            <div><strong>D</strong> — See diagram</div>
            <div><strong>ER</strong> — End radius</div>
            <div><strong>G</strong> — See diagram</div>
            <div><strong>TAPPED</strong> — Thread size for T parts</div>
            <div><strong>D-D DIA.</strong> — Thru-hole diameter for H parts</div>
          </div>
        </div>
      </div>
    </section>
