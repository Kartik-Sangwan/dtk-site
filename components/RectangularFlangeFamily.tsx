...
export default function RectangularFlangeFamily({ subcategory, priceByPartNo = {} }: { subcategory: string; priceByPartNo?: Record<string, number> }) {
  const [q, setQ] = useState("");

  const rows = useMemo(() => {
    const s = q.trim().toLowerCase();
    const baseRows = rectangularFlanges.filter((r) =>
      Object.prototype.hasOwnProperty.call(priceByPartNo, r.part)
    );
    if (!s) return baseRows;

    // allows searching "DF 15" or "DF15"
    ...
    <section className="w-full">
      ...
      <div className="rounded-2xl border border-slate-200/70 bg-white/70 p-6 shadow-sm backdrop-blur">
        <div className="grid gap-8 md:grid-cols-[1.2fr_1fr] md:items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Rectangular Flange</h1>
            <p className="mt-3 text-gray-700">
              NFPA rectangular flanges. Search a part number to view dimensions and tapped thread.
            </p>
      ...
      <div className="mt-8 overflow-hidden rounded-2xl border border-slate-200/70 bg-white/70 shadow-sm backdrop-blur">
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-white/90 backdrop-blur">
              <tr className="border-b border-slate-200">
                ...
                <TipTh label="UF" tip="UF dimension (see diagram)" />
                <TipTh label="TF" tip="TF dimension (see diagram)" />
                <TipTh label="FB" tip="4-FB (hole) / bolt hole size (see diagram)" />
                <TipTh label="E" tip="E dimension (see diagram)" />
                <TipTh label="R" tip="R dimension (see diagram)" />
                <TipTh label="F" tip="F thickness (see diagram)" />
                <TipTh label="AA" tip="AA dimension (see diagram)" />
                <TipTh label="TAPPED" tip="Tapped thread size" />
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.part} className="border-b border-slate-100 hover:bg-slate-50/60">
                  ...
                  <td className="px-4 py-3 text-gray-800">{r.UF}</td>
                  <td className="px-4 py-3 text-gray-800">{r.TF}</td>
                  <td className="px-4 py-3 text-gray-800">{r.FB}</td>
                  <td className="px-4 py-3 text-gray-800">{r.E}</td>
                  <td className="px-4 py-3 text-gray-800">{r.R}</td>
        ...
        <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
          ...
          <div className="mt-3 grid gap-2 text-sm text-gray-700 sm:grid-cols-2 lg:grid-cols-3">
            <div><strong>UF</strong> — UF dimension (see diagram)</div>
            <div><strong>TF</strong> — TF dimension (see diagram)</div>
            <div><strong>FB</strong> — 4-FB (hole) / bolt hole size</div>
            <div><strong>E</strong> — E dimension (see diagram)</div>
            <div><strong>R</strong> — R dimension (see diagram)</div>
            <div><strong>F</strong> — Thickness</div>
            <div><strong>AA</strong> — AA dimension (see diagram)</div>
            <div><strong>TAPPED</strong> — Tapped thread size</div>
          </div>
        </div>
      </div>
    </section>
