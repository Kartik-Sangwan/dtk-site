...
export default function EyeBracketFamily({ subcategory, priceByPartNo = {} }: { subcategory: string; priceByPartNo?: Record<string, number> }) {
  const [q, setQ] = useState("");

  const rows = useMemo(() => {
    const s = q.trim().toLowerCase();
    const baseRows = eyeBrackets.filter((r) => Object.prototype.hasOwnProperty.call(priceByPartNo, r.part));
    ...
    <section className="w-full">
      ...
      <div className="rounded-2xl border border-slate-200/70 bg-white/70 p-6 shadow-sm backdrop-blur">
        <div className="grid gap-8 md:grid-cols-[1.2fr_1fr] md:items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Eye Brackets</h1>
            <p className="mt-3 text-gray-700">
              NFPA eye mounting brackets. Search a part number to view all dimensions.
              <span className="ml-1 text-gray-600">
                (“H” denotes heavy base.)
              </span>
            </p>
            ...
            <div className="mt-6">
              ...
              <input
                ...
                placeholder='Try "DEB-20" or "DEB-20H"'
      ...
      <div className="mt-8 overflow-hidden rounded-2xl border border-slate-200/70 bg-white/70 shadow-sm backdrop-blur">
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-white/90 backdrop-blur">
              <tr className="border-b border-slate-200">
                ...
                <TipTh label="CB" tip="CB (see diagram)" />
                <TipTh label="CD" tip="CD DIA. (pin hole diameter)" />
                <TipTh label="DD" tip="DD DIA. (mounting hole diameter)" />
                <TipTh label="E" tip="E (base length / overall length per diagram)" />
                <TipTh label="F" tip="F (see diagram)" />
                <TipTh label="FL" tip="FL (flange length)" />
                <TipTh label="LR" tip="LR (left radius)" />
                <TipTh label="M" tip="M (see diagram)" />
                <TipTh label="MR" tip="MR (mount radius)" />
                <TipTh label="R" tip="R (see diagram)" />
        ...
        <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
          ...
          <div className="mt-3 grid gap-2 text-sm text-gray-700 sm:grid-cols-2 lg:grid-cols-3">
            <div><strong>CB</strong> — CB (see diagram)</div>
            <div><strong>CD</strong> — CD DIA. (pin hole diameter)</div>
            <div><strong>DD</strong> — DD DIA. (mounting hole diameter)</div>
            <div><strong>E</strong> — E (base length / overall length per diagram)</div>
            <div><strong>F</strong> — F (see diagram)</div>
            <div><strong>FL</strong> — FL (flange length)</div>
            <div><strong>LR</strong> — LR (left radius)</div>
            <div><strong>M</strong> — M (see diagram)</div>
            <div><strong>MR</strong> — MR (mount radius)</div>
            <div><strong>R</strong> — R (see diagram)</div>
          </div>
        </div>
      </div>
    </section>
